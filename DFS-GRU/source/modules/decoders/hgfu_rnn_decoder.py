...
class RNNDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.tgt_emb_prj_weight_sharing:
            ...
            self.tgt_word_prj.weight = self.embedder.weight
        ...
        self.output_layer = nn.Sequential(
            ...
            nn.Linear(self.out_input_size, self.embed_size, bias=True),
            self.tgt_word_prj)
    ...
    def decode(self, input, state, is_training=False):  # 这里是每一个时间步执行一次，注意这里batch_size特指有效长度，即当前时间步无padding的样本数
        ...
        fusion_memory_copy = torch.cat([dialg_context_copy, fact_context_copy],
                                       1)  # (batch_size , 2 , 2*rnn_hidden_size)
        ...
        weight_gen = self.sigmoid(
            self.fc1_copy(fusion_context_copy) + self.fc2_copy(query_copy) + self.fc3_copy(input)).squeeze(
            1)  # (batch_size,1)
        ...
        if is_training:
            ...
            logit = self.output_layer(out_input)
            log_prob_gen = self.softmax(logit*self.x_logit_scale)
            log_prob_dialog = log_prob_gen.new_zeros(size=(log_prob_gen.size(0), 1, self.output_size),
                                                     dtype=torch.float)  # （batch_size, 1, vocab_size）
            log_prob_fact = log_prob_gen.new_zeros(size=(log_prob_gen.size(0), 1, self.output_size),
                                                   dtype=torch.float)  # （batch_size, 1, vocab_size）
            log_prob_dialog.scatter_(2, state.dialog_wordindex.unsqueeze(1),
                                     dialog_copy_p.unsqueeze(1))  # (batch_size, 1, vocab_size)
            log_prob_fact.scatter_(2, state.fcor_wordindex.unsqueeze(1),
                                   fact_copy_p.unsqueeze(1))  # (batch_size, 1, vocab_size)
            log_prob_copy = log_prob_dialog + log_prob_fact

            log_prob = torch.mul(log_prob_gen, weight_gen.unsqueeze(1)) + torch.mul(log_prob_copy,
                                                                                    1 - weight_gen.unsqueeze(1))
    ...
    def forward(self, inputs, state, dialog_wordindex, fcor_wordindex):
        ...
        out_inputs = inputs.new_zeros(
            ...
            dtype=torch.float)  # （batch_size, tgt_max_len, 4*rnn_hidden_size）
        ...
        logits = self.output_layer(out_inputs)
        log_probs_gen = self.softmax(logits*self.x_logit_scale)
        log_probs_dialog = log_probs_gen.new_zeros(size=(batch_size, max_len, self.output_size),
                                                   dtype=torch.float)  # （batch_size, max_len, vocab_size）
        log_probs_fact = log_probs_gen.new_zeros(size=(batch_size, max_len, self.output_size),
                                                 dtype=torch.float)  # （batch_size, max_len, vocab_size）

        log_probs_dialog.scatter_(2, dialog_wordindex.unsqueeze(1).repeat(1, max_len, 1),
                                  out_probs_copy_x)  # (batch_size,  max_len, vocab_size)
        log_probs_fact.scatter_(2, fcor_wordindex.unsqueeze(1).repeat(1, max_len, 1),
                                out_probs_copy_f)  # (batch_size, max_len, vocab_size)
