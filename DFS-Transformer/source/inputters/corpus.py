...
class Corpus(object):
    ...
    def build_vocab(self, data):
        ...
        data: ``List[Dict]``
        """
        field_data_dict = {}#不分享词表{self.SRC:[src1,src2,src3.......](原生未分词)，self.TGT:[tgt1,tgt2,tgt3.......](原生未分词)}/
                            #分享词表{self.SRC:[src1,src2,src3.......，tgt1,tgt2,tgt3.......](原生未分词)}
        for name in data[0].keys():#src/tgt
            field = self.fields.get(name)#src/tgt
            if isinstance(field, TextField):
    ...
    def build_examples(self, data):
        ...
        Args
        ----
        data: ``List[Dict]``_raw数据形式（原生文本未分词）：list[Dict[str]]------[{src:…,tgt:…,cue:……},{src:…,tgt:…,cue:[…,…,…,…]}…………]
        """
        examples = []
        ...
        return examples   #examples数据形式（索引，已分词，已加前后缀）：list[Dict[list]]------[{src:…,tgt:…,cue:[[],[],[]…]},{src:…,tgt:…,cue:[[],[],[]…]}…………]
    ...
    def build(self):
        ...
        print("Reading data ...")
        #_raw数据形式（原生文本未分词）：list[Dict]------[{src:…,tgt:…,cue:[[],[],[]…]},{}…………]
        ...
        vocab = self.build_vocab(train_raw)#{"src":{"itos": self.itos,"embeddings": self.embeddings},"tgt":{"itos": self.itos,"embeddings": self.embeddings},"cue":{"itos": self.itos,"embeddings": self.embeddings}}
    ...
    def __init__(self,
        ...
        if self.share_vocab:
            ...
            self.CUE = TextField(tokenize_fn=tokenize,
                                 embed_file=embed_file)
            self.KEY = TextField(tokenize_fn=tokenize,
                                 embed_file=embed_file)

        if self.with_label:
            self.INDEX = NumberField()
            self.fields = {'src': self.SRC, 'tgt': self.TGT, 'key':self.KEY, 'cue': self.CUE, 'index': self.INDEX}
        else:
            self.fields = {'src': self.SRC, 'tgt': self.TGT, 'key':self.KEY, 'cue': self.CUE}
    ...
    def read_data(self, data_file, data_type="train"):
        ...
        with open(data_file, "r", encoding="utf-8") as f:
            ...
            for line in f:
                count=count+1
                if self.with_label:
                    src, tgt, key_words, knowledge, label = line.strip().split('\t')[:5]
                    filter_knowledge = []
                    cue_count = 0
                    for sent in knowledge.split(''):
                        cue_count+=1
                        filter_knowledge.append(' '.join(sent.split()[:self.max_len]))
                    if cue_count<8 and not data_type=='test':
                        print("< 8 knoewledge example occured")
                        continue
                    data.append({'src': src, 'tgt': tgt, 'key': key_words, 'cue': filter_knowledge, 'index': label})
                else:
                    src, tgt,key_words, knowledge = line.strip().split('\t')[:4]
                    filter_knowledge = []
                    cue_count = 0
                    for sent in knowledge.split(''):
                        filter_knowledge.append(' '.join(sent.split()[:self.max_len]))
                        cue_count+=1
                    if cue_count<4 and not data_type=='test':
                        print("< 4 knoewledge example occured")
                        continue
                    data.append({'src': src, 'tgt': tgt, 'key': key_words,'cue':filter_knowledge})
