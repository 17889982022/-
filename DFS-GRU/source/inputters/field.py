...
class TextField(Field):
    ...
    def build_vocab(self, texts, min_freq=0, max_size=None):#[src1,src2,src3.......，tgt1,tgt2,tgt3.......](原生未分词为列表-->字符串)
        """
        build_vocab
        """
        def flatten(xs):
        ...
        counter = Counter()
        for string in tqdm(texts):#[src1,src2,src3.......，tgt1,tgt2,tgt3.......](原生未分词为列表-->字符串)
        ...
        words_and_frequencies.sort(key=lambda tup: tup[1], reverse=True)#[('you', 2), ('?', 1), ('I', 1), ('love', 1), ('understand', 1)………………]
    ...
    def build_word_embeddings(self, embed_file):
        ...
        if isinstance(embed_file, list):
            ...
            print("{} words have pretrained {}-D word embeddings (coverage: {:.3f})".format( \
                    cover, dim, rate))
        return embeds  #[[* dim],[* dim],[* dim],[* dim]…………………………]（len(self.stoi）embed_file中没有的单词embedding会设为0。
