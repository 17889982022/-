...
import torchsnooper
from source.utils.FLOPs_counter import print_model_parm_flops
...
def evaluate(model, data_iter, verbose=False):
    ...
    with torch.no_grad():
        for inputs in data_iter:
            ...
            # print_model_parm_flops(model, inputs, detail=True)
            mm.update(metrics)
            ss.extend(scores.tolist())
            # batch_count_ops, count_params = profile(model, inputs=(input,))
        #     count_ops += batch_count_ops
        # print('FLOPs: {}, Params: {}'.format(count_ops/len(data_iter), count_params))
    return mm, ss
...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for batch_id, inputs in enumerate(self.train_iter, 1):
            ...
            if batch_id % self.log_steps == 0:
                ...
                print('learning rate:', self.optimizer._optimizer.param_groups[0]["lr"])
    ...
    def save(self, is_best=False):
        ...
        train_state = {"epoch": self.epoch,
                       ...
                       "n_current_steps": self.optimizer.n_current_steps}
        if self.lr_scheduler is not None:
    ...
    def load(self, file_prefix):
        ...
        self.optimizer._optimizer.load_state_dict(train_state_dict["optimizer"])
        self.optimizer.n_current_steps = train_state_dict['n_current_steps']
...
def LR_tuning(epoch):
    base_lr = 0.0001
    if epoch <= 10:
        return base_lr + epoch * 0.00005
    else:
        return 0.0006 - (epoch - 10) * 0.00005
...
def evaluate_generation(generator,
    ...
    elapsed = time.time() - start_time
    print('time:',elapsed/len(data_iter))
